...
client.on('message', async msg => { // eslint-disable-line
    ...
    if (!msg.content.startsWith(A)) return undefined;
    const args = msg.content.split(' ');
    const searchString = args.slice(1).join(' ');
    const url = args[1];
    const serverQueue = queue.get(msg.guild.id);
   
    if(msg.content.startsWith(`${A}play`)){
...
Please enter a number between 1-10 on,a Song select!`)
    ...
    } else if(msg.content.startsWith(`${A}skip`)) {
     ...
     else if (msg.content.startsWith(`${A}leave`)){
        if(!msg.member.voiceChannel){
   ...
   else if(msg.content.startsWith(`${A}song`)){
       if(!serverQueue){
...
`)
   ...
   else if(msg.content.startsWith(`${A}volume`)){
       if(!serverQueue){
   ...
   else if(msg.content.startsWith(`${A}queue`)){
       if(!serverQueue){
...
Playing: ${serverQueue.songs[0].title}`)
   ...
   else if(msg.content.startsWith(`${A}stop`)){
   ...
   else if(msg.content.startsWith(`${A}run`)){
       if(serverQueue && !serverQueue.playing){
   ...
   else if(msg.content.startsWith(`${A}mute`)){
       if(!serverQueue){
   ...
   else if(msg.content.startsWith(`${A}unmute`)){
       if(!serverQueue){
...
,`
 ...
 There is Commands Just For ! DoMaBOY | Bot By: ! DoMaBOY#1362
